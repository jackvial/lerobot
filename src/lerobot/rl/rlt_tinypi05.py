...
"""TinyPI0.5 RLT policy: frozen tinypi05 VLA + lightweight RL-token actor/critic.

This mirrors `lerobot.rl.rlt_pi05` but targets the smaller `tinypi05` backbone,
which uses the simpler base `PI05Pytorch` API (no subtask generation, no FAST
tokens, plain `OBS_LANGUAGE_TOKENS` only). The reusable RLT pieces
(`RLTokenAutoencoder`, `RLTActorHead`, `RLTCriticHead`, `RLTCriticEnsemble`,
losses, target-network helpers, checkpoint helpers) are imported directly from
`rlt_pi05` to avoid duplication.
"""
...
@PreTrainedConfig.register_subclass("tinypi05_rlt")
@dataclass
class TinyPI05RLTConfig(TinyPI05Config):
    """TinyPI0.5 wrapper config for RLT.

    The base `TinyPI05Config` controls the frozen VLA architecture; the
    `rlt_*` fields configure the lightweight RL-token autoencoder plus
    actor/critic heads layered on top.
    """
    ...
    # uniformly across `pi05_rlt` and `tinypi05_rlt`.
    pi05_checkpoint: str | None = None

    @classmethod
    def from_base_config(cls, base_config: TinyPI05Config, **overrides: Any) -> "TinyPI05RLTConfig":
        kwargs: dict[str, Any] = {}
        for field_info in fields(TinyPI05Config):
...
class TinyPI05RLTPolicy(TinyPI05Policy):
    """RLT wrapper around `TinyPI05Policy`.

    The frozen tinypi05 VLA stays unchanged. With no actor checkpoint loaded,
    `predict_action_chunk` is an exact pass-through of the VLA reference chunk
    (matching the behavior of `PI05RLTPolicy`).
    """

    config_class = TinyPI05RLTConfig
    name = "tinypi05_rlt"

    def __init__(self, config: TinyPI05RLTConfig, **kwargs):
        super().__init__(config, **kwargs)
        self.config: TinyPI05RLTConfig
    ...
    def compute_prefix_embeddings(
        ...
        Note: the base `PI05Pytorch.embed_prefix` returns 1D `att_masks` (the
        prefix-LM attention pattern for `make_att_2d_masks`); we pass
        `prefix_pad_masks` to the RL-token autoencoder for key-padding masking.
        """
    ...
    def forward(
        ...
        raise NotImplementedError(
            "TinyPI05RLTPolicy currently supports offline `rlt_embedding` training only."
        )
